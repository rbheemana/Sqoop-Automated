...
def generateScripts(template_path, app_src_path, app_wrk_path,datasource,sourceSystem, jdbcReference, dbUserName,env_ver, appName,sub_app, tableItems, sqoopParamFilePath, sqoopparams,key_store,database):
        ...
        for line in open(template_path+"/templates/create_parquet_as_of_date.hql"):
        ...
        for tableItem in tableItems:
            ...
            partitionType=tableParams[0].strip() if paramsLength > 0 else ""
            partitionCol =tableParams[1].strip() if paramsLength > 1 else ""
            splitBy      =tableParams[2].strip() if paramsLength > 2 else ""
            numMappers   =tableParams[3].strip() if paramsLength > 3 else 4
            
            #Raw hive table name if hive table name needs to be different from source table name
            stgTableName=tableParams[4] if paramsLength > 4 else tableName
            if stgTableName == "":
                stgTableName = tableName
            stgDestDBLevel=tableParams[5].strip().lower() if paramsLength > 5 else ""
            if stgDestDBLevel ==  "" or stgDestDBLevel== "pub" :
                stgDestDBLevel = " "
            else:
                #hiveDestDBLevel = "_"+hiveDestDBLevel
                stgDestDBLevel = stgDestDBLevel
            print "generate.py             -> stgDestDBLevelToken "+ stgDestDBLevel
            #Target hive table name if hive table name needs to be different from source table name
            hiveTableName=tableParams[6] if paramsLength > 6 else tableName
            if hiveTableName == "":
                hiveTableName = tableName
            hiveDestDBLevel=tableParams[7].strip().lower() if paramsLength > 7 else ""
            if hiveDestDBLevel ==  "" or hiveDestDBLevel== "pub" :
                hiveDestDBLevel = " "
            else:
                #hiveDestDBLevel = "_"+hiveDestDBLevel
                hiveDestDBLevel = hiveDestDBLevel
            print "generate.py             -> hiveDestDBLevelToken "+ hiveDestDBLevel
            partitionColumnExp=tableParams[8].strip().lower() if paramsLength > 8 else ""
            if partitionType.lower() == 'ext' and partitionColumnExp == "":
            ...
            if partitionType.lower() == 'ext':
                       extPartitions=partitionCol.split(':')
                       partitionCol=extPartitions[0]
                       if (len(extPartitions) < 2):
                          dataType='string'
                       else:
                          dataType=extPartitions[1]
                       partitionToken = "partition ( " + partitionCol + " ) "
                       partitionColumnSelectToken = partitionColumnExp
                       partitionColumnToken = partitionCol
                       partitionedByToken = "partitioned by ( " + partitionCol + " " + dataType + " )"
                       partitionDataType=dataType
                       createPartitionColumn = partitionCol + " " + dataType
            if tableFound == "true":
                createStgColumnToken=""
                if not partitionColumnToken == "" and not partitionType.lower() == 'ext':
                ...
                tableJobProperties = tableJobProperties.replace("${whereColumnToken}",whereCol)
                ...
                if partitionType.lower() == 'ext':
                   tableJobProperties = tableJobProperties.replace("${columnsToken}","${columns_without_partition}")
                else:
                  tableJobProperties = tableJobProperties.replace("${columnsToken}","${columns_without_partition}${partition_column_select}")
                ...
                outfile.close()
                subprocess.call(["chmod", "777",wfPath+"/" + TtableName +".properties"])
                ...
                outfile.close()
                subprocess.call(["chmod", "777",app_src_path+"/hive/" + TtableName +"_create_stg.hql"])
                ...
                subprocess.call(["chmod", "777",app_src_path+"/hive/" + TtableName +"_create_parquet.hql"])
